...
### PERF note (2026-07-15): caption obstacle probes
Caption-dodging queries the spatial index per labeled visible edge per frame;
draw-cost p95 moved ~8.1 -> ~9.2ms on the 6k benchmark (60Hz floor unaffected,
93% of frames still under the 8.3ms/120Hz budget). If ProMotion matters later:
cache caption fractions per board revision and invalidate on drag.
